...
export class EuropaParkBase extends Park {
  ...
  */
  constructor(options = {}) {
    ...
    options.apiBaseOld = process.env.EUROPAPARK_OLD;
    ...
    options.langoptions = `{'en', 'nl', 'de', 'fr'}`; // Accidentally found that EP provides Dutch data
    ...
    return fetch(this.config.loginurl,
        {
          method: 'POST',
          headers: {
            'user-agent': 'EuropaParkApp/10.1.0 (Android)',
  ...
  */
  async getPOIS() {
    return await this.cache.wrap('poi', async () => {
      ...
      return fetch(this.config.apiBaseOld +
        `api/v1/latest/en/live/${checksum}`,
      {
        headers: {
          'user-agent': 'EuropaParkApp/10.1.0 (Android)',
          'accept-language': 'de',
