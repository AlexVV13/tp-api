...
* @class
*/
export class Rulantica extends Park {
  /**
  * Create a new EuropaPark Park object
  * @param {object} options
  */
  constructor(options = {}) {
    options.name = options.name || 'Rulantica';
    ...
    options.langoptions = options.langoptions || `{'en', 'nl', 'de', 'fr'}`; // Accidentally found that EP provides Dutch data, lmao
  ...
  *
  * const park = new tpapi.park.Rulantica();
  *
  * park.loginEP().then((token) => {
  * console.log(token)
  * });
  * @return {string} Rulantica refresh token
  */
  async loginEP() {
  ...
  * console.log(pois)
  * });
  * @return {string} All Rulantica ride POIS without queuetimes
  */
  async buildRidePOI() {
    return await this.cache.wrap('ridedata', async () => { // Rebuilding the ride object each time is SLOOOOOWWWW
      ...
      poiData.pois.forEach((ride) => { // Data includes Rulantica, however, they're listed as slides, so not defining anything special here.
        if (ride.type === 'attraction' && ride.code !== null && ride.scopes.indexOf('rulantica') === 0) { // Return rides and pois which haven't null
          const area = 'Rulantica'; // Really, this is the strangest empire thing ever
  ...
  /**
  * Build Rulantica restaurant object
  ...
  * console.log(pois)
  * });
  * @return {string} All Rulantica restaurant POIS
  */
  async buildRestaurantPOI() {
    return await this.cache.wrap('restdata', async () => { // Rebuilding the ride object each time is SLOOOOOWWWW
      ...
      poiData.pois.forEach((ride) => {
        if (ride.type === 'gastronomy' && ride.code !== null && ride.scopes.indexOf('rulantica') === 0) { // Return rides and pois which haven't null
          const area = 'Rulantica'; // Default, Hotel is included as well which adds new areas
  ...
  * console.log(pois)
  * });
  * @return {string} All Rulantica static POIS without queuetimes
  */
  async buildStaticPOI() {
    return await this.cache.wrap('staticdata', async () => { // Rebuilding the ride object each time is SLOOOOOWWWW
      const poiData = await this.getPOIS(); // Get the POI Data
      ...
      const poi = {};
      poiData.pois.forEach((ride) => { // Data includes Rulantica, however, they're listed as slides, so not defining anything special here.
        if (ride.type === 'sight' && ride.code !== null && ride.scopes.indexOf('rulantica') === 0) { // Return rides and pois which haven't null
  ...
  /**
  * Build Rulantica Merchandise object
  ...
  * console.log(pois)
  * });
  * @return {string} All Rulantica Merchandise POIS
  */
  async buildMerchandisePOI() {
    return await this.cache.wrap('merchdata', async () => { // Rebuilding the ride object each time is SLOOOOOWWWW
      ...
      const poi = {};
      poiData.pois.forEach((ride) => {
        if (ride.type === 'shopping' && ride.code !== null && ride.scopes.indexOf('rulantica') === 0) { // Return rides and pois which haven't null
          const area = 'Rulantica'; // Default, Hotel is included as well which adds new areas
  ...
  /**
  * Get All Queues of Rulantica
  * This data contains all the Queues in Rulantica, attached with pois above.
  * @example
  * import tpapi from '@alexvv13/tpapi';
  *
  * const park = new tpapi.park.Rulantica();
  ...
  * console.log(queue)
  * });
  * @return {string} Rulantica POIS with queues
  */
  async getQueue() {
  ...
  /**
  * Build Rulantica Service object
  ...
  * console.log(pois)
  * });
  * @return {string} All Rulantica Services POIS
  */
  async buildServicePOI() {
    return await this.cache.wrap('servicedata', async () => { // Rebuilding the ride object each time is SLOOOOOWWWW
      ...
      const poi = {};
      poiData.pois.forEach((ride) => {
        if (ride.type === 'service' && ride.code !== null && ride.scopes.indexOf('rulantica') === 0) { // Return rides and pois which haven't null
          const area = 'Rulantica'; // Default, Hotel is included as well which adds new areas
  ...
  /**
  * Get All Operating Hours of Rulantica
  * This data contains all the Operating Hours in Rulantica, fetched with currentyear.
  * @example
  * import tpapi from '@alexvv13/tpapi';
  *
  * const park = new tpapi.park.Rulantica();
  ...
  * console.log(hours)
  * });
  * @return {string} Rulantica park hours
  */
  async getOpHours() {
    const token = await this.refreshEP();
