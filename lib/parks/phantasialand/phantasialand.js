...
export class Phantasialand extends Park {
  ...
  */
  constructor(options = {}) {
    ...
    options.supportsrideschedules = 'trues';
      ...
      return fetch(`${this.config.waitTimesURL}?loc=${RandomLocation.latitude},${RandomLocation.longitude}&compact=true&access_token=${this.config.apiKey}`,
          ...
          .then((ride) => {
            ...
            ride.forEach((rideData) => {
              ...
              if (rideData.opening && rideData.closing) {
                openTime = rideData.opening;
                closingTime = rideData.closing;
                opType = queueType.operating;
              } else {
                openTime = undefined;
                closingTime = undefined;
                opType = queueType.closed;
              }

              if (poi[rideData.poiId]) {
                const rideobj = {
                  name: poi[rideData.poiId].name,
                  id: `Phantasialand_${poi[rideData.poiId].id}`,
                  state: state,
                  waitTime: waitTime,
                  active: active,
                  changedAt: rideData.updatedAt,
                  location: poi[rideData.poiId].location,
                  meta: poi[rideData.poiId].meta,
                  schedule: {
                    openTime: openTime,
