  ...
  .then(rideData => {
    ...
    rideData.hits.hit.forEach(ride => {
      if(ride.fields.category === 'attraction' && ride.fields.hide_in_app !== 'false'){
        if(ride.fields.alternateid.contains('singlerider')){
          const singlerider = 'true', //Initial single rider implementation, since the park is closed I can't really do more then this anyway
        } else {
          const singlerider = 'false',
        }
