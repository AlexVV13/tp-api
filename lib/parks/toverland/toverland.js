    ...
    return fetch(`${this.config.apiBase}/park/ride/operationInfo/list`,
        ...
        .then((ride) => {
          ...
          ride.forEach((poi) => {
            ...
            } else if (state === 'Open' && !poi.last_waiting_time) {
              waitTime = '0';
              active = true;
              state = queueType.operating;
            ...
            if (Object.keys(poi.opening_times).length < 1) {
              ...
              openTime = moment(poi.opening_times.start, 'YYYY-MM-DD HH:mm').format();
              closingTime = moment(poi.opening_times.end, 'YYYY-MM-DD HH:mm').format();
